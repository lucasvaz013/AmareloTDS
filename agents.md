...
- **Events** — micro‑interações por `clickid+step` (`click_steps.events`); **não** viram conversão/payout/CAPI. Helpers padrão: `offer_revealed` observa `[data-ytds-offer]` (fallback só com um `.delay-hidden`) e `checkout_click` observa `{link:N}`/`[data-ytds-checkout]`; custom continua coexistindo (`^[a-z][a-z0-9_]{0,63}$`).
