...
export function buildFolderRow(folderName, showWeight, folderConfig) {
    var frag = cloneTemplate('tpl-folder-row');
    folderConfig = folderConfig || {};
    var item = frag.querySelector('.flow-path-item');
    if (item) {
        item.dataset.mvt = JSON.stringify(
            folderConfig.mvt || { enabled: false, tests: [] }
        );
        item.dataset.links = JSON.stringify(folderConfig.links || []);
    }
    ...
    if (weightInput) {
        weightInput.className = 'form-control flow-step-weight';
        weightInput.value = folderConfig.weight || '';
    }
    ...
    if (modeBtn) {
        modeBtn.classList.add('flow-step-mode');
        modeBtn.dataset.mode = folderConfig.loadtype || 'base';
        var icon = modeBtn.querySelector('i');
        if (icon && modeBtn.dataset.mode === 'direct') {
            icon.className = 'bi bi-hdd-network';
        }
    }

    var mvtPanel = frag.querySelector('.flow-mvt-panel');
    if (mvtPanel) {
        renderMvtPanel(
            mvtPanel,
            folderConfig.mvt || { enabled: false, tests: [] }
    ...
    var linksPanel = frag.querySelector('.flow-links-panel');
    if (linksPanel) {
        renderLinksPanel(linksPanel, folderConfig.links || []);
    }
