...
// Picking a verified domain is the normal path: it was registered, pointed and published
// under Domains already, so nothing here has to be typed or spelled correctly.
document.getElementById('verified-domain-picker')?.addEventListener('change', function () {
    var picker = this;
    var domain = picker.value;
    if (!domain) return;
    ...
    var container = document.getElementById('domains_container');
    container.insertAdjacentHTML('beforeend', buildDomainRow(domain, loadingHtml));
    checkDomainStatus(container.querySelector('.domain-item:last-child'));
    if (window.syncDomainWhiteSections) window.syncDomainWhiteSections();

    // Dropped from the list so the same domain cannot be picked twice.
    for (var j = picker.options.length - 1; j >= 0; j--) {
        if (picker.options[j].value === domain) picker.remove(j);
    }
    picker.value = '';
    if (picker.options.length <= 1) {
        picker.disabled = true;
        picker.options[0].textContent = 'No verified domains available';
    }
});
