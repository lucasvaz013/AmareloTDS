<?php
/**
 * AmareloTDS PHP Client Library
 * 
 * Simple include file for connecting external PHP sites to AmareloTDS.
    ...
    private function sendRequest($params) 
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'User-Agent: AmareloTDS-PHP-Client/1.0'
            ],
        ...
        if ($curl_error) {
            $this->log("AmareloTDS cURL Error: " . $curl_error);
            return null;
        }
        
        if ($http_code !== 200) {
            $this->log("AmareloTDS HTTP Error: " . $http_code);
            return null;
        }
        
        $decoded = json_decode($response, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->log("AmareloTDS JSON Error: " . json_last_error_msg(). "\n" . $response);
            return null;
        }
