# Установка на shared hosting

Установка AmareloTDS на обычный виртуальный (shared) хостинг представляет из себя операцию copy-paste: выберите домен, загрузите файлы и откройте админку в браузере.

## Требования к хостингу

- PHP 8.2 или новее;
- расширения PHP `curl`, `sqlite3`;
- возможность записи из PHP в каталог AmareloTDS;
- HTTPS для выбранного домена или поддомена.

Расширения `mbstring`, `xml`, `zip` и `maxminddb` желательны, но не обязательны для базовой установки.

## Установка

1. Скачайте ZIP ветки `production` и распакуйте его:

   ```text
   https://github.com/lucasvaz013/AmareloTDS/archive/refs/heads/production.zip
   ```

2. Загрузите в каталог сайта только содержимое папки `code/`. Каталог `code/` уже является полным дистрибутивом.
3. Включите в панели хостинга PHP 8.2+, SQLite и HTTPS.
4. Откройте админку и завершите настройку:

   ```text
   https://example.com/admin/
   ```

AmareloTDS можно разместить как в корне сайта, так и в подпапке. Для установки в подпапку добавьте её к адресу админки, например `https://example.com/tds/admin/`.

## Важно для безопасности

Админка открывается напрямую и не требует `.htaccess`. Однако перед запуском рабочего трафика веб-сервер должен закрывать прямой доступ к базе SQLite, настройкам и логам. Для режима Direct Load он также должен направлять служебные URL в `index.php`.

На Apache и LiteSpeed для этого используется `.htaccess`, а на nginx — эквивалентные правила сайта. Если хостинг уже направляет отсутствующие URL в `index.php`, Direct Load будет работать без дополнительных действий. Если такого правила нет и изменить конфигурацию нельзя, Direct Load недоступен — используйте Base Load.

Это настройка веб-сервера, а не отдельная установка AmareloTDS. Готовые правила приведены в разделе [«Обязательные правила веб-сервера»](hosting-panels.md#обязательные-правила-веб-сервера).
