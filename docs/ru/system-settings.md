...
- **Plugins** — включение валютных источников и VPN/proxy-детекторов, предпочтительные валюты и режим решения VPN/proxy. **Any positive** считает посетителя VPN/proxy, если хотя бы один включённый детектор вернул положительный результат. **Majority** требует положительных результатов от более чем половины детекторов, вернувших корректный ответ. При одном ответившем детекторе режимы равнозначны; если не ответил ни один, AmareloTDS считает посетителя VPN/proxy.
- **Updates** — проверка и установка обновления AmareloTDS и обновление GeoBases.
